...
# Setup
Name "foobox 视频面板"
OutFile "foobox-video_x64.cn.${VER_DATE}.exe"
# VerInfo
...
本程序不包含 foobar2000 程序及 foobox 7 主题，安装前您应该已安装有 foobar2000 播放器 (x64版) 并整合了 foobox 7 DUI 主题."
...
Section "foo_input_ffmpeg 解码器封装组件" fooffmpeg
	...
	File ".\cn\video\vx64\profile\user-components-x64\foo_input_ffmpeg\*.*"
	${If} $noConfig = 0
		SetOutPath "$ProfileDir\configuration"
		File ".\cn\video\vxcommon\profile\configuration\foo_input_ffmpeg.dll.cfg"
	${EndIf}
...
Section "视频面板 foo-youtube" VideoYoutube
	...
	${If} $noConfig = 0
		...
		File ".\cn\video\vx64\profile\theme.fth"
		SetOutPath "$ProfileDir\configuration"
		${If} $noAdmin = 0
			File /oname=foo_youtube.dll.cfg ".\cn\video\vxcommon\profile\configuration\foo_youtube_admin.dll.cfg"
		${Else}
			File /oname=foo_youtube.dll.cfg ".\cn\video\vxcommon\profile\configuration\foo_youtube_noadmin.dll.cfg"
		${EndIf}
	${EndIf}
	SetOutPath "$INSTDIR\themes"
	File ".\cn\video\vx64\themes\foobox + 简介 + 视频 (youtube).fth"
	File ".\cn\video\vx64\themes\foobox + 视频 (youtube).fth"
	SetOutPath "$ProfileDir\foo_youtube"
	File /r ".\common\vx64\profile\foo_youtube\*.*"
	File /r ".\cn\video\vx64\profile\foo_youtube\*.*"
	...
	SetOutPath "$INSTDIR"
	${If} $PortableMode = 0
        File "/oname=LavFilters助手.bat" ".\cn\video\vxcommon\lavassist\LavFilters助手_0.bat"
	${Else}
        File "/oname=LavFilters助手.bat" ".\cn\video\vxcommon\lavassist\LavFilters助手_1.bat"
	;${Else}
	;	File "/oname=LavFilters助手.bat" ".\cn\video\vxcommon\lavassist\LavFilters助手_2.bat"
    ${EndIf}
SectionEnd

Section /o "视频面板 foo-mpv" VideoMPV
	SetOutPath "$ProfileDir\user-components-x64\foo_mpv"
	File /r ".\cn\video\vx64\profile\user-components-x64\foo_mpv\*.*"
	${If} $noConfig = 0
		SetOutPath "$ProfileDir\configuration"
		File ".\cn\video\vxcommon\profile\configuration\foo_mpv.dll.cfg"
	${EndIf}
	SetOutPath "$INSTDIR\themes"
	File ".\cn\video\vx64\themes\foobox + 简介 + 视频 (mpv).fth"
	File ".\cn\video\vx64\themes\foobox + 视频 (mpv).fth"
	SetOutPath "$ProfileDir\mpv"
	File /r ".\cn\video\vxcommon\profile\*.*"
SectionEnd
...
!insertmacro MUI_HEADER_TEXT "选项及注意事项" "重要提示: 务必先安装 64 位版本的 foobar2000 及 foobox 主题."
...
${EndIf}
${NSD_CreateCheckbox} 10u 60u 90% 10u "不安装任何配置文件"
Pop $CfgCheckbox
${If} $noConfig = 1
	${NSD_Check} $CfgCheckbox
${EndIf}
${NSD_CreateLabel} 20u 72u 90% 30u "如果勾选, 视频组件 input_ffmpeg, foo_youtube, foo_mpv 的 cfg 文件将不会安装. 谨慎, 不确定请勿勾选!"
nsDialogs::Show
FunctionEnd
