...
# Setup
Name "foobox 视频面板"
OutFile "foobox8-video_x64.cn.${VER_DATE}.exe"
# VerInfo
...
Section "foo_input_ffmpeg 解码器封装组件" fooffmpeg
	...
	File ".\cn\vx64\profile\user-components-x64\foo_input_ffmpeg\*.*"
	${If} $noConfig = 0
...
Section "视频面板 foo-youtube" VideoYoutube
	...
	File ".\cn\vx64\themes\foobox8 + 简介 + 视频(youtube).fth"
	File ".\cn\vx64\themes\foobox8 + 视频(youtube).fth"
...
Section /o "视频面板 foo-mpv" VideoMPV
	...
	File /r ".\cn\vx64\profile\user-components-x64\foo_mpv\*.*"
	${If} $noConfig = 0
		SetOutPath "$ProfileDir\configuration"
		File ".\cn\vxcommon\profile\configuration\foo_mpv.dll.cfg"
	${EndIf}
	SetOutPath "$INSTDIR\themes"
	File ".\cn\vx64\themes\foobox8 + 简介 + 视频(mpv).fth"
	File ".\cn\vx64\themes\foobox8 + 视频(mpv).fth"
