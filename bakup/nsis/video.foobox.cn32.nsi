...
# Setup
Name "foobox 视频面板"
OutFile "foobox-video_x86.cn.${VER_DATE}.exe"
# VerInfo
...
Section "foo_input_ffmpeg 解码器封装组件" fooffmpeg
	...
	File ".\cn\video\vx86\profile\user-components\foo_input_ffmpeg\*.*"
	${If} $noConfig = 0
...
Section "视频面板 foo-youtube" VideoYoutube
	...
	${If} $noConfig = 0
		...
		File ".\cn\video\vx86\profile\theme.fth"
		SetOutPath "$ProfileDir\configuration"
		${If} $noAdmin = 0
	...
	File ".\cn\video\vx86\themes\foobox-7 + 简介 + 视频 (youtube).fth"
	File ".\cn\video\vx86\themes\foobox-7 + 视频 (youtube).fth"
	IfFileExists "$ProfileDir\foobox\version6" 0 +6
	SetOutPath "$INSTDIR\themes"
	File ".\cn\video\vx86_v6\themes\foobox-6 + 简介 + 视频 (youtube).fth"
	File ".\cn\video\vx86_v6\themes\foobox-6 + 视频 (youtube).fth"
	SetOutPath "$ProfileDir"
	File ".\cn\video\vx86_v6\profile\theme.fth"
	SetOutPath "$ProfileDir\foo_youtube"
	File /r ".\common\vx86\profile\foo_youtube\*.*"
	File /r ".\cn\video\vx86\profile\foo_youtube\*.*"
...
Section /o "视频面板 foo-mpv" VideoMPV
	...
	File /r ".\cn\video\vx86\profile\user-components\foo_mpv\*.*"
	${If} $noConfig = 0
		SetOutPath "$ProfileDir\configuration"
		File ".\cn\video\vxcommon\profile\configuration\foo_mpv.dll.cfg"
	${EndIf}
	SetOutPath "$INSTDIR\themes"
	File ".\cn\video\vx86\themes\foobox-7 + 简介 + 视频 (mpv).fth"
	File ".\cn\video\vx86\themes\foobox-7 + 视频 (mpv).fth"
	IfFileExists "$ProfileDir\foobox\version6" 0 +4
	SetOutPath "$INSTDIR\themes"
	File ".\cn\video\vx86_v6\themes\foobox-6 + 简介 + 视频 (mpv).fth"
	File ".\cn\video\vx86_v6\themes\foobox-6 + 视频 (mpv).fth"
